...
export function interfaceCounts(markdown) {
  ...
  return {
    ...
    stateTopics: matching(topics, /\/(joint_states|tcp_pose|tcp_state|jacobian)\//),
    servoJTopics: matching(topics, /\/servoj\//),
    servoLTopics: matching(topics, /\/servol\//),
    moveActions: matching(actions, /\/move_absj\//),
    motionServices: matching(services, /\/(movej_by_path|move_l|move_l_relative|get_cartesian_state)\//),
    teachServices: matching(services, /\/cartesian_teach\//),
